...
flowchart TD
    Q1{Do you already\nuse a platform?}
    Q1 -->|OpenStack| OS[Deploy a VM via Nova\nor use Trove DBaaS]
    Q1 -->|Proxmox| PX[Run DB in a VM\nor LXC container]
    Q1 -->|Kubernetes| K8S[Use a database operator\nCloudNativePG · KubeBlocks]
    Q1 -->|None / simple| DS[Dedicated server\nPhysical · VM · Docker]
...
flowchart LR
    ...
    App1 -->|"upload / download\nvia HTTP"| OS
    App2 -->|"upload / download\nvia HTTP"| OS
    App3 -->|"read datasets\nwrite results"| OS
    OS --> CLD
    OS --> SH
...
flowchart TD
    ...
    OS --> SAAS[Cloud-managed\nAWS S3 · Azure Blob · GCS]
    OS --> SH[Self-hosted\nSwift · Garage]

    DB --> MS[Managed Cloud Service\nAWS RDS, Azure Database …]
    DB --> SELF[Self-hosted]
    SELF --> DS[Dedicated Server\nPhysical · VM · Docker]
    SELF --> OST[OpenStack\nNova VM · Trove DBaaS]
    SELF --> PX[Proxmox\nVM · LXC container]
    SELF --> K8S[Kubernetes\nwith Operator]
    K8S --> CNPG[CloudNativePG\nPostgreSQL specialist]
    K8S --> KB[KubeBlocks\nMulti-engine]
