...
What this documentation cannot fully plan for is what is in place already: existing servers with some application, a Virtual Machine infrastructure... Which can be used as a base for the set-up, if well documented and supported. Using these can be a major plus for a new solution.
...
services:
  main:
    ...
    depends_on:
      db:
        condition: service_healthy   # Upgraded from a bare list to a map so we can
                                     # specify 'service_healthy' — now Compose waits
                                     # for the healthcheck below to pass before starting 'main'.
...
It's possible to have a bind mounts, i.e. a volume attached to a specific path on the host, by using an external volume and creating the volume using the docker command
