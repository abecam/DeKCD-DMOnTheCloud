...
ls -> list the content of a directory, **ls -al** -> include all file (so including hidden files (starting by ".")) and as a detailed list.

cd -> go to a directory (cd .. to go to the parent folder, cd / for the root folder, cd ~ for the user home folder)

ps -> list the running processes, **ps -edf** or **ps -aux** -> list the running processes and their owner
...
mount -> allow to use a file system. For instance, a USB key needs to be mounted before it can be accessed. The root folder to temporary file system is often /mnd/. Modern Unix system will automount some medias.

pwd/whoami -> **pwd** return the current path, **whoami** the current user.
...
::: {.callout-note}
Knowing eval is important for security reason. The presence of an eval in a suspicious script calls for caution.
:::

curl is also good to know.

free/df/dh -> know how many resources are available 
...
rsync -> remote synchronizing of files and directories. It allows to save several defined files and/or directories on another host without a dedicated backup tool. Together with a database dump it could serve as a backup solution. In that case it is important to have a monitoring of the process to ensure it is working.
