...
Note that Linux is not the only Unix, OpenBSD and FreeBSD being good alternative, but all Unix are very similar for their core usage and structure. The package managers will differ a lot, but also differ on Linux between Debian-based Linux (dpkg), RedHat-based Linux (rpm, yum, now dnf) and Ubuntu-based Linux (apt/apt-get), apk (Alpine Linux), as well as sandboxed or universal package managers like Flatpak and Snap, to list the main ones.
...
* **vi / vim / emacs / nano** -> most of the time we work on servers/containers with a text terminal. Being able to edit a file is often needed. Vi, nano or Emacs are powerful text-based editors present on most Linux distributions, though minimal container images might only provide vi.
...
man -> show the manual for a command. man should be an automatic reflex for any less-known usage. It is absolutely normal to forget options or syntax.
If you forget a command name, keep a Linux cheat sheet around. A quick internet search will find many good one-page PDFs.

ln -> create a link and know the difference between:

a hard link: direct link to the file, removed when the file is removed, shares permissions

a symbolic link (symlink): independent object pointing to a path, survives file moves/removal, has its own permissions

For setting up applications, we generally use symbolic links, often to control permissions or switch versions/configurations easily.

chown / chmod -> change the owner of a file or folder, change permissions. Be familiar with:

rwxrwxrwx, r-x------

numeric modes like 755, 644

examples: chmod o+r filename, chmod -R 755 directory, chown -R user:group directory

The pipe (|) should be well understood, as well as I/O redirections (>, >>, <).
If this is unclear, look for “Unix pipes and redirection” — it is a core concept.

grep -> search for a string in files, often combined with another command using a pipe (|)

find -> search for files by name, type, size, or date. Often combined with grep or xargs

mount / umount -> attach a filesystem so it can be accessed. For instance, a USB key must be mounted before use. Temporary mount points are often under /mnt or /media. Modern systems often automount removable media.

pwd / whoami -> pwd returns the current path, whoami returns the current user

kill -> kill sends a signal to a process.
The default signal is SIGTERM (kill -TERM pid or simply kill pid), which asks the process to terminate cleanly.
If the process does not respond (hung, infinite loop), SIGKILL (kill -KILL pid or kill -9 pid) will force termination.

SIGKILL is unsafe and should only be used after SIGTERM has failed.

Signals have numeric values (POSIX), e.g. -15 for SIGTERM, -9 for SIGKILL.

eval -> powerful and dangerous command that executes its arguments as a shell command. It allows building commands dynamically in scripts, but can easily introduce security vulnerabilities.

::: {.callout-note}
Knowing eval is important for security reasons. The presence of eval in a suspicious script calls for caution.
:::

curl / wget -> interact with HTTP/HTTPS services from the command line. Very useful to test APIs, web services, or download files.

free / df / du -> monitor resources:
...
ssh / scp -> connect to a remote machine (ssh) and copy files over SSH (scp). Essential for remote servers.
If unfamiliar, search for “SSH basics”.

systemctl -> part of systemd (daemon manager). Web servers (Apache, Nginx), databases, and applications usually run as daemons managed by systemctl.
Commands to know:
...
A graceful reload allows existing connections to finish before restarting (important for web servers). Keep in mind that application-level sessions (CMS, APIs) may still be interrupted depending on design.

apachectl configtest / nginx -t -> test the configuration of Apache or Nginx before restarting. This reduces the risk of breaking a production service, but does not guarantee the configuration behaves as intended.

rsync -> efficient synchronization of files and directories, locally or remotely. Often used for backups or deployments. Combined with database dumps, it can form a simple backup strategy. Monitoring is important to ensure it actually runs.

docker / docker-compose (if applicable) -> many modern web applications are containerized. Even a basic understanding of docker ps, docker logs, docker restart is very useful.
If this is new, look for a short “Docker for beginners” guide.

Folders you should know

/ -> filesystem root

/bin -> essential system binaries (basic commands)

/usr -> user-space applications and libraries
/usr/bin -> most user commands and applications

/etc -> system-wide configuration files (very important for web servers, services, users)

/var -> variable data
/var/log -> system and application logs (first place to look when something fails)
...
/opt -> optional or third-party software (often used for manually installed applications)
